...
YOLO::YOLO(const utils::InitParameter& param) : m_param(param)
{
    // input
...
bool YOLO::init(const std::vector<unsigned char>& trtFile)
{
...
// copy to device
void YOLO::copy(const std::vector<cv::Mat>& imgsBatch)
{
#if 0 
    ...
    for (size_t i = 0; i < imgsBatch.size(); i++)
    {
        // std::cout << imgsBatch[i].rows << ", "  << imgsBatch[i].cols << "\n" ;
        // std::cout << m_param.src_h << ", " << m_param.src_w << "\n";
...
void YOLO::postprocess(const std::vector<cv::Mat>& imgsBatch)
{
#if 0 // valid
    {
...
std::vector<std::vector<utils::Box>> YOLO::getObjectss() const
{
...
void YOLO::setParameters(utils::InitParameter& initParameters){
}


void YOLO::drawInfo(std::vector<cv::Mat>& imgsBatch){
